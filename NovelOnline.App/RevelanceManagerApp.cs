...
namespace NovelOnline.App
{
    public class RevelanceManagerApp : BaseApp<Relevance>
    {
        ...
        public void Assign(string operatorId,string type, string firstId, string[] secIds)
        {
            Assign(operatorId,type, secIds.ToLookup(u => firstId));
        }

        public void Assign(string operatorId,string key, ILookup<string, string> idMaps)
        {
            DeleteBy(key, idMaps);
            UnitWork.AddRange((from sameVals in idMaps
        ...
        {
            DeleteBy(key, idMaps);
            UnitWork.AddRange<Relevance>((from sameVals in idMaps
                                          from value in sameVals
                                          select new Relevance
                                          {
                                              Key = key,
        ...
        //public RevelanceManagerApp(IUnitWork unitWork) : base(unitWork)
        {
        }
    }
}
